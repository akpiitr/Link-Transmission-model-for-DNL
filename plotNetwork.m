...
%open figure
if isempty(fig_num)
    handle_fig = figure('Position',[200,50,1000,900]);
else
...
if show_label
    id=[1:length(x)]';
    %% node texts
    t0=text(x(id),y(id),num2str(id),'Color',[1 0 0],'VerticalAlignment','Bottom','HorizontalAlignment','Left','FontWeight','bold','Clipping','on','hittest','off');
    %%
    upX=x(strN);
    downX=x(endN);
    upY=y(strN);
    ...
    %% link texts
    t1=text(cx(bl),cy(bl),num2str(id(bl)),'Color',[0 0 0],'VerticalAlignment','Bottom','HorizontalAlignment','Left','FontWeight','bold','Clipping','on','hittest','off');
    t2=text(cx(tl),cy(tl),num2str(id(tl)),'Color',[0 0 0],'VerticalAlignment','Top','HorizontalAlignment','Left','FontWeight','bold','Clipping','on','hittest','off');
    t3=text(cx(br),cy(br),num2str(id(br)),'Color',[0 0 0],'VerticalAlignment','Bottom','HorizontalAlignment','Right','FontWeight','bold','Clipping','on','hittest','off');
    t4=text(cx(tr),cy(tr),num2str(id(tr)),'Color',[0 0 0],'VerticalAlignment','Top','HorizontalAlignment','Right','FontWeight','bold','Clipping','on','hittest','off');
    handle_txt=[t0];
end
